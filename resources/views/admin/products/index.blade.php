                    ...
                    @foreach ($products as $product)
                        <tr class="bg-white border-b">
                            ...
                            <td class="px-4 py-3 flex justify-center space-x-2">
                                <a href="{{ route('admin.products.edit', $product->id) }}" class="bg-yellow-400 text-white px-3 py-1 rounded-lg hover:bg-yellow-500">EDIT</a>
                                <button onclick="openModal({{ $product->id }}, '{{ $product->stock }}', '{{ $product->nama_produk }}')" 
                                    class="bg-blue-500 text-white px-3 py-1 rounded-lg hover:bg-blue-600">
                                    UPDATE STOCK
                                </button>
                                <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ url('admin/products/' . $product->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
